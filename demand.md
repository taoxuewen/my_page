...
## [D12] 项目 GitHub 首页 README　—　2026-06-09
- **原始诉求**：给这个项目的 GitHub 仓库首页加一个 README。
- **背景**：仓库根目录原本没有 README，访客打开 GitHub 看不到项目介绍。
- **讨论结论 / 实现**：新建根目录 `README.md`——含项目简介、设计风格说明、在线应用清单（含状态）、技术栈、快速开始（含依赖与 API Key 配置）、项目结构、活文档导览、作者信息。API Key 用环境变量示意，不暴露真实密钥。
- **影响范围**：新增 `README.md`（纯文档，不影响运行）。
- **状态**：已交付（2026-06-09）。
